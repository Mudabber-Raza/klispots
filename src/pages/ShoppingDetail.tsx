...
import { ArrowLeft, Phone, MapPin, Clock, Star, Heart, Share2, Calendar, ShoppingBag, Users, Car, CreditCard, Globe, Award, BookOpen, Lightbulb } from 'lucide-react';
import Header from '@/components/layout/Header';
import Footer from '@/components/layout/Footer';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { getShoppingVenueById } from '@/data/shopping';
...
import { getShoppingRecommendations } from '@/utils/recommendationEngine';

const ShoppingDetail = () => {
  const { id } = useParams();
  const venue = getShoppingVenueById(id || '');
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="container mx-auto px-4 py-6">
          <div className="text-center">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">Shopping Venue Not Found</h1>
            <p className="text-gray-600 mb-8">Sorry, we couldn't find the shopping venue you're looking for.</p>
            <Link to="/shopping" className="text-blue-600 hover:text-blue-800">
              ← Back to Shopping
            </Link>
          </div>
        </main>
  ...
  const parseOperatingHours = (hoursString) => {
    ...
    const cleanHours = cleanText(hoursString).replace(/â€"/g, '-');
    
    const dayHours = cleanHours.split(/(?=Monday:|Tuesday:|Wednesday:|Thursday:|Friday:|Saturday:|Sunday:)/)
      .filter(Boolean)
      .map(dayTime => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          {venue.phone_number && (
            <Button onClick={handleCall} size="lg" className="flex-1 min-w-48 bg-purple-600 hover:bg-purple-700">
              <Phone className="h-5 w-5 mr-2" />
              Call Mall
            </Button>
          )}
          <Button onClick={handleDirections} variant="outline" size="lg" className="flex-1 min-w-48 border-purple-600 text-purple-600 hover:bg-purple-50">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="shadow-lg border-0">
              ...
              <CardContent className="space-y-8">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <ScoreBar label="Retail Variety & Store Quality" score={venue.retail_variety_and_store_quality_score} />
                  <ScoreBar label="Dining & Food Court" score={venue.dining_and_food_court_score} />
                  <ScoreBar label="Entertainment & Recreation" score={venue.entertainment_and_recreation_score} />
                  <ScoreBar label="Facilities & Amenities" score={venue.facilities_and_amenities_score} />
                  <ScoreBar label="Location & Accessibility" score={venue.accessibility_and_location_score} />
                  <ScoreBar label="Value & Pricing" score={venue.value_and_pricing_score} />
              ...
              <CardContent className="p-8">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    {venue.best_time_for_family_shopping && (
                      <div className="p-6 bg-rose-50 rounded-xl border border-rose-200">
                        ...
                        <p className="text-sm text-rose-600">{cleanText(venue.best_time_for_family_shopping)}</p>
                      </div>
                    )}
                    
                    {venue.best_time_for_couples && (
                      <div className="p-6 bg-blue-50 rounded-xl border border-blue-200">
                        <h3 className="font-semibold mb-3 flex items-center justify-center gap-2 text-blue-700">
                          💑 Couples
                        </h3>
                        <p className="text-sm text-blue-600">{cleanText(venue.best_time_for_couples)}</p>
                      </div>
                    )}

                    {venue.best_time_for_solo_shopping && (
                      <div className="p-6 bg-purple-50 rounded-xl border border-purple-200">
                        <h3 className="font-semibold mb-3 flex items-center justify-center gap-2 text-purple-700">
                          👤 Solo Shopping
                        </h3>
                        <p className="text-sm text-purple-600">{cleanText(venue.best_time_for_solo_shopping)}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    {venue.weekend_vs_weekday && (
                      <div className="p-6 bg-yellow-50 rounded-xl border border-yellow-200">
                        <h3 className="font-semibold mb-3 text-yellow-700 text-center">📅 Weekend vs Weekday</h3>
                        <p className="text-sm text-yellow-600">{cleanText(venue.weekend_vs_weekday)}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <Card className="shadow-lg border-0">
              <CardContent className="p-8">
                ...
                <div className="space-y-6">
                  ...
                  {venue.target_demographics && (
                    <div className="p-6 bg-purple-50 rounded-xl border border-purple-200">
                      <h3 className="font-semibold text-purple-800 mb-3">🎯 Target Audience</h3>
                      <p className="text-purple-700">{cleanText(venue.target_demographics)}</p>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card className="shadow-lg border-0">
              <CardContent className="p-8">
                ...
                <p className="text-gray-600 mb-4">Discover three more shopping areas in {venue.city}</p>
                <div className="p-6 bg-blue-50 rounded-xl border border-blue-200">
                  <p className="text-blue-700">Coming soon: Recommendations for similar shopping venues in your area.</p>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-lg border-0">
              <CardContent className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-xl">Entertainment & Facilities</h3>
                </div>
                
                <div className="space-y-4">
                  {venue.entertainment_and_recreation && (
                    <div className="p-4 bg-blue-50 rounded-xl">
                      <h4 className="font-semibold text-blue-800 mb-2">Entertainment & Recreation</h4>
                      <div className="text-sm text-blue-700">
                        {parseListText(venue.entertainment_and_recreation).map((item, index) => (
                  ...
                  {venue.facilities_and_amenities && (
                    <div className="p-4 bg-green-50 rounded-xl">
                      <h4 className="font-semibold text-green-800 mb-2">Facilities & Amenities</h4>
                      <div className="text-sm text-green-700">
                        {parseListText(venue.facilities_and_amenities).map((item, index) => (
                          <div key={index} className="mb-1">• {item}</div>
                        ))}
                      </div>
                    </div>
                        ...
                        <div key={index} className={`p-3 rounded-lg ${
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <div className={`text-sm text-right ${
                              isToday ? 'text-purple-600 font-medium' : 'text-gray-600'
            ...
            <Card className="shadow-lg border-0">
              <CardContent className="p-6">
                <h3 className="font-bold text-xl mb-4">Visit Information</h3>
                <div className="space-y-4">
                  ...
                  {venue.mall_theme && (
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <h4 className="font-medium mb-2">Mall Theme</h4>
                      <p className="text-sm text-gray-600">{cleanText(venue.mall_theme)}</p>
                    </div>
                  )}
                </div>
              </CardContent>
      ...
      {venue && (
        <RecommendationSection
          recommendations={getShoppingRecommendations(venue, id || '')}
          title="You Might Also Like"
          subtitle="Discover more amazing shopping destinations"
        />
      )}
